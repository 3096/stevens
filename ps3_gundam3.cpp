...
bool add_title(void)
{
 // title
 STDSTRING p1;
 p1 += TEXT("[PS3] Dynasty Warriors Gundam 3");
 ...
 p3 += TEXT("2. You need 7 GB free space.\n");
 p3 += TEXT("3. 3 GB for game + 4 GB extraction.");
...
bool extract(LPCTSTR pname)
{
 ...
 if(doLDA) {
    ...
    PS3::DYNASTY_WARRIORS::UnpackANS(file.c_str());
   }
 if(doLDB) {
    STDSTRING file = pathname;
