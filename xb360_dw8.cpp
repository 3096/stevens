...
bool add_title(void)
{
 // title
 STDSTRING p1;
 p1 += TEXT("[XBOX360] Dynasty Warriors 8");
 ...
 p3 += TEXT("3. 5 GB for game + 15 GB extraction.");
...
bool extract(LPCTSTR pname)
{
 ...
 // questions
 if(doIDX1) doIDX1 = YesNoBox("Process LINKDATA.IDX file?");
 if(doIDX2) doIDX2 = YesNoBox("Process LINKDATA2.IDX file?");
 if(doEDAT) doEDAT = YesNoBox("Process XBox360 DLC (EDAT) files?");
 if(doPACK) doPACK = YesNoBox("Process extracted (IDXZRC and IDXOUT) files?");
 if(doG1T_) doG1T_ = YesNoBox("Process texture (G1T) files?");
 if(doG1M_) doG1M_ = YesNoBox("Process model (G1M) files?");
 ...
 if(doIDX1) {
    ...
    XB360::DYNASTY_WARRIORS::UnpackIDX(file1.c_str());
   }

 // unpack archive #2
 if(doIDX2) {
    STDSTRING file2 = pathname;
    file2 += TEXT("LINKDATA2.IDX");
    XB360::DYNASTY_WARRIORS::UnpackIDX(file2.c_str());
   }

 // rename DLC files
 if(doEDAT)
    BatchFileExtensionRename(pathname, TEXT(".edat"), TEXT(".idxout"), true);
