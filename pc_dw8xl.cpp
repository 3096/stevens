...
#pragma region "LINKDATA_UNPACKING"
...
bool add_title(void)
{
 // title
 STDSTRING p1;
 p1 += TEXT("[PC] Dynasty Warriors 8: Xtreme Legends");
 ...
 p3 += TEXT("2. You need ~37 GB free space.\n");
 p3 += TEXT("3. 16 GB for game + 21 GB extraction.\n");
...
bool extract(LPCTSTR pname)
{
 ...
 if(doRES) doRES = YesNoBox("Process files in RES and RES_EMP folders?\nSay 'No' if you have already done this before.");
 ...
 if(doIDX) {
    ...
    DYNASTY_WARRIORS::UnpackIDX_B(file.c_str(), ENDIAN_LITTLE);
   }
 ...
 if(doRES)
   {
    ...
    respath2 << pathname << TEXT("RES_EMP\\");
    ...
    // process 2nd folder
    cout << "Processing RES_EMP\\.BIN files..." << endl;
    filelist.clear();
    BuildFilenameList(filelist, TEXT(".BIN"), respath2.str().c_str());
    for(size_t i = 0; i < filelist.size(); i++) {
        wcout << "Processing file " << (i + 1) << " of " << filelist.size() << ": " << filelist[i] << "." << endl;
        if(!DYNASTY_WARRIORS::IDXOUT_FileExtract(filelist[i].c_str(), ENDIAN_LITTLE)) return false;
       }
