...
bool add_title(void)
{
 // title
 STDSTRING p1;
 p1 += TEXT("[PS3] Dragon Quest Heroes (BLJM61256)");
 ...
 p3 += TEXT("3. 2 GB for game + 4 GB extraction.");
...
bool extract(LPCTSTR pname)
{
 ...
 if(doDLC) doDLC = YesNoBox("Process PS3 DLC (BIN.GZ) files?");
 ...
 // rename DLC files
 if(doDLC) PS3::DYNASTY_WARRIORS::UnpackDLC_TypeC(pathname.c_str());
