...
bool extract(LPCTSTR pname)
{
 ...
 // questions
 if(doCAT) doCAT = YesNoBox("Merge BIN files? Say 'No' if you have already done so.");
 if(doIDX) doIDX = YesNoBox("Process LINKDATA.IDX file?\nThis archive contains character models and props.");
 if(doDLC) doDLC = YesNoBox("Process PS3 DLC (DAT) files?");
 ...
 if(doCAT) {
    ...
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66600"));
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66601"));
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66602"));
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66603"));
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66604"));
    filelist.push_back(pathname + TEXT("LINKDATA.BIN.66605"));
    cout << "Merging BIN files... please wait... it's like over 4 GB lol." << endl;
    if(!MergeBinaryFiles(filelist, (pathname + TEXT("LINKDATA.BIN"))))
       return error("Failed to merge BIN files.");
   }
 ...
 // rename DLC files
 if(doDLC) PS3::DYNASTY_WARRIORS::UnpackDLC_TypeD(pathname.c_str());
